...
const availableFilters = {
    year: (festivals, year) =>
        festivals.filter(festival => festival.title.slice(-4) === year),
    continent: (festivals, continent) =>
        festivals.filter(festival => festival.continent === continent),
    search: (festivals, searchInput) => {
        const re = new RegExp(searchInput, "i");
        const filteredFests = festivals.filter(festival =>
            re.test(festival.title)
        );
...
const getYearsOptions = festivalList => {
    const years = festivalList
        .map(festival => festival.title.slice(-4))
        .filter((elem, pos, arr) => {
            return arr.indexOf(elem) === pos;
        });
    const sortedYears = years.sort().reverse();
    return sortedYears.concat("cancel");
};
        ...
        case SAVE_SEARCH_INPUT:
            if (action.payload) {
                if (state.filters.filter(f => f.type === "search").length > 0) {
                    ...
                    filters = [
                        ...filters,
                        { type: "search", args: action.payload }
                    ];
                    const filteredFestivals = filterFestivals(
                        state.allFestivals,
                        filters
