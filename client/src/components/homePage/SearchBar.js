        ...
        if (event.keyCode === 13 && this.props.filteredFestivals.length === 0) {
            axios
                .post("/api/v1/requests", {
                    request: { festival_name: event.target.value }
                })
                .then(response => {
                    if (response.data.created_at) {
                        ...
                        this.setState({
                            requestMessage:
                                "this festival has already been requested!"
                        });
                    }
        ...
        return (
            <div className="searchInput animated fadeIn delay-4">
                ...
                <input
                    ...
                    value={
                        this.props.filters.filter(f => f.type === "search")[0]
                ...
                />
                <p
                    className={`${this.props.filteredFestivals.length === 0 &&
                        "showMessage"} requestMessage`}
                >
                    This festival does not exist! Press enter to send a request!
                </p>
            </div>
        );
    }
